...
static float train_batch(
    ...
    optimizer_step(model, learning_rate / (float)batch_size);
    return total_loss;
}
...
void train(Model* model, float* train_x, float* train_y, int dataset_size, int epochs, float learning_rate) {
    if (model == NULL || model->layer_count == 0 || train_x == NULL || train_y == NULL || dataset_size <= 0) {
        return;
    }
...
int train_loader(Model* model, DataLoader* loader, int epochs, float learning_rate) {
    if (model == NULL || loader == NULL || model->layer_count == 0 || epochs <= 0 || learning_rate <= 0.0f) {
        return 0;
    }
    if (dataloader_input_size(loader) != model->input_size || dataloader_output_size(loader) != model->output_size) {
        return 0;
    }
